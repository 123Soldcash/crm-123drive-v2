...
export function TwilioCallWidget({ phoneNumber, contactName, contactId, propertyId, primaryTwilioNumber }: TwilioCallWidgetProps) {
  ...
  return (
    <>
      {/* When primaryTwilioNumber is set, use a plain button — no Popover at all */}
      {primaryTwilioNumber ? (
        <Button
          variant="ghost"
          size="sm"
          ...
          title={`Call ${contactName} (using default: ${formatPhone(primaryTwilioNumber)})`}
        >
        ...
        <Popover open={selectorOpen} onOpenChange={setSelectorOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              className="h-7 w-7 p-0 hover:bg-green-50 rounded-full"
              title={`Call ${contactName} — select a number`}
            >
              <Phone className="h-3.5 w-3.5 text-green-600" />
            </Button>
          </PopoverTrigger>
