...
export function ContactManagement({ propertyId }: ContactManagementProps) {
  ...
  const handleDelete = (contactId: number) => {
    if (confirm("Are you sure you want to delete this contact? This will also delete all associated phones, emails, and communication logs.")) {
      ...
      {!contacts || contacts.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            No contacts available. Add a contact to start tracking communication.
          </CardContent>
        </Card>
      ) : (
